...
built for an older version of agda, so doesn't work (reflection details
has changed), but it does provide a peek preview of lens codegen
possibilties in agda. note that besides not currently working, it also
takes forever to type check even to the point of failure ;)
